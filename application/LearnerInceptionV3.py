...
class LearnerInceptionV3(Learner):
    def learn(self):
        if not os.path.exists("tmp/model/model.json"):
            ...
            hist = model.fit_generator(
                self.dataset.generate_arrays_from_file_a(category='training', batch_size=self.FLAGS.train_batch_size),
                steps_per_epoch=1,
                epochs=1500,  # 1000000
                validation_data=self.dataset.generate_arrays_from_file_a(category='validation',
    ...
    def predict(self):
        ...
        (X, Y) = self.dataset.get_batch_data(category='testing', batch_size=500)
